# =============================================================================
# Gastrulation Dynamics — MEDAKA Focused Analysis
# =============================================================================
#
# Core analyses: Velocity/Acceleration, Speed, Straightness, Persistence,
# Neighbor coordination, Directional decomposition, MSD, Thickness, Nuclear size
#
# Outputs:  analysis_output/medaka_dynamics_*.pdf & .csv
# =============================================================================
...
# Contraction artefact: from this timepoint onwards, the embryo contracts and
# nuclear counts / sizes become unreliable.  Data is shown transparently but
# quantitative conclusions are extracted only from tp <= CONTRACTION_TP.
CONTRACTION_TP <- 500
CONTRACTION_MIN <- CONTRACTION_TP * FRAME_INTERVAL_MIN  # in minutes

INPUT_DIR   <- "oriented_medaka_ultrack"
OUTPUT_DIR  <- "analysis_output_medaka"
NUCLEAR_DIR <- "nuclei_stats_medaka"
...
# Voxel sizes from TIFF metadata (nuclear_stats_global.tsv): dXY=1.05152 um, dZ=1.05263 um.
# Near-isotropic (<0.2% anisotropy), so a single scale factor is applied after rotation.
VOXEL_SIZE_UM  <- 1.05152  # um per pixel (XY)
...
cat("\n")
cat(strrep("=", 70), "\n")
cat("  GASTRULATION DYNAMICS — MEDAKA\n")
cat(strrep("=", 70), "\n")
...
spots <- fread(file.path(INPUT_DIR, "oriented_tracks_medaka.csv"), showProgress = FALSE)
setDF(spots)
...
if (nrow(ingr_candidates) >= 10) {
  ...
  INGRESSION_CENTER_THETA <- ifelse(!is.na(INGRESSION_THETA_LM), INGRESSION_THETA_LM, 79)
  INGRESSION_CENTER_PHI   <- ifelse(!is.na(INGRESSION_PHI_LM), INGRESSION_PHI_LM, -94)
...
# Use data.table for speed — dplyr group_by over 140K tracks is too slow
setDT(spots)
...
# Velocity computation — shift by VELOCITY_LAG frames for temporal averaging
...
if (is.na(MARGIN_THETA_LANDMARK)) {
  MARGIN_THETA_LANDMARK <- quantile(spots$THETA_DEG, 0.90)
  cat(sprintf("  WARNING: No landmark, using 90th pctile theta=%.1f\n", MARGIN_THETA_LANDMARK))
}

ZONE_MARGIN_TOP  <- MARGIN_THETA_LANDMARK - ZONE_MARGIN_WIDTH
ZONE_PREMARG_TOP <- ZONE_MARGIN_TOP - ZONE_PREMARG_WIDTH

ingr_lat_lo <- INGRESSION_CENTER_THETA - INGRESSION_RADIUS
ingr_lat_hi <- INGRESSION_CENTER_THETA + INGRESSION_RADIUS
...
spots <- spots %>%
  mutate(
    ...
    zone = factor(case_when(
      THETA_DEG > MARGIN_THETA_LANDMARK ~ "Sub-marginal",
...
track_metrics <- track_metrics %>%
  mutate(
    ...
    zone = factor(case_when(
      mean_theta > MARGIN_THETA_LANDMARK ~ "Sub-marginal",
...
# MSD alpha over time
time_windows <- seq(0, max(spots$FRAME) - 30, by = 30)
alpha_time <- lapply(time_windows, function(t0) {
  sub <- spots %>% filter(FRAME >= t0 & FRAME <= t0 + 30)
  if (length(unique(sub$TRACK_ID)) < 100) return(NULL)
  m <- compute_msd(sub, max_lag = 15, n_sample = 2000)
  if (nrow(m) < 3) return(NULL)
  data.frame(time_min = (t0 + 15) * FRAME_INTERVAL_MIN, alpha = fit_alpha(m))
}) %>% bind_rows()
...
cat(sprintf("  %s neighbor pairs computed\n", format(nrow(coord_results), big.mark = ",")))
...
# Thickness = depth range of blastoderm cells at each location
THICK_EPOCH <- 15  # frames per bin
THICK_DTHETA <- 3
THICK_DPHI   <- 5
...
# Global thickness over time by zone
...
# Ingression cluster vs control thickness (using spatial proximity, not zone label)
thickness_ingr <- spots %>%
  mutate(epoch = floor(FRAME / 10) * 10, time_epoch = epoch * FRAME_INTERVAL_MIN,
         location = case_when(
           dist_to_ingr < INGRESSION_RADIUS ~ "Ingression cluster",
           THETA_DEG >= ingr_lat_lo & THETA_DEG <= ingr_lat_hi &
             dist_to_ingr >= INGRESSION_RADIUS ~ "Same-lat control",
           TRUE ~ "Other"
         )) %>%
  filter(location != "Other") %>%
  group_by(epoch, time_epoch, location) %>%
  summarise(
    thickness_iqr = IQR(SPHERICAL_DEPTH, na.rm = TRUE),
    thickness_p90 = diff(quantile(SPHERICAL_DEPTH, c(0.05, 0.95), na.rm = TRUE)),
    thickness_range = diff(range(SPHERICAL_DEPTH, na.rm = TRUE)),
...
# 1c: Acceleration magnitude distribution
ACCEL_BINS <- 120
...
# 1d: Turning angle — smoothing sensitivity validation
# Overlays turning-angle densities at multiple smoothing levels.
# If distributions collapse toward 0° with smoothing → noise artifact.
# If they persist → real biological turning behaviour.
...
# 1h: Speed stability across smoothing levels (control)
...
# 1f: Velocity components by zone over time
comp_time <- spots_vel %>%
  filter(!is.na(zone)) %>%
  mutate(time_bin = floor(time_min / 5) * 5) %>%
  group_by(time_bin, zone) %>%
...
# 1g: Velocity components by latitude
...
save_pdf(fig1, "medaka_dynamics_01_velocity.pdf", width = 16, height = 22)
...
p2a <- ggplot(track_metrics %>% filter(!is.na(zone)),
  ...
  scale_fill_manual(values = zone_colors) +
  labs(title = "Track straightness by zone",
       subtitle = "1 = straight (directed), 0 = tortuous",
       x = NULL, y = "Straightness") +
  theme_dynamics() + theme(legend.position = "none",
                           axis.text.x = element_text(angle = 20, hjust = 1))

# 2b: Turning angle by zone
p2b <- ggplot(track_metrics %>% filter(!is.na(mean_turning), !is.na(zone)),
              aes(x = zone, y = mean_turning, fill = zone)) +
  geom_violin(alpha = 0.6, color = NA) +
  geom_boxplot(width = 0.15, outlier.size = 0.3, fill = "white", alpha = 0.7) +
  scale_fill_manual(values = zone_colors) +
  labs(title = "Mean turning angle by zone",
       subtitle = "Low = persistent, high = random",
       x = NULL, y = "Turning angle (deg)") +
  theme_dynamics() + theme(legend.position = "none",
                           axis.text.x = element_text(angle = 20, hjust = 1))

# 2c: Confinement by movement type
...
fig2 <- (p2a + p2b) / (p2c + p2d) +
  plot_annotation(
    ...
    subtitle = sprintf("N = %s tracks with >= %d frames", format(nrow(track_metrics), big.mark = ","), MIN_TRACK_LENGTH),
    theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14),
                  plot.subtitle = element_text(hjust = 0, size = 10, color = "grey50"))
  )
save_pdf(fig2, "medaka_dynamics_02_track_properties.pdf", width = 14, height = 10)
...
save_pdf(fig3, "medaka_dynamics_03_neighbors.pdf", width = 14, height = 10)
...
p4d <- ggplot(mvt_prop, aes(x = zone, y = pct, fill = movement_type)) +
  ...
  theme_dynamics() + theme(axis.text.x = element_text(angle = 20, hjust = 1))
...
save_pdf(fig4, "medaka_dynamics_06_direction.pdf", width = 14, height = 10)
...
p5a <- ggplot(msd_global, aes(x = lag_min, y = mean_msd)) +
  ...
  labs(title = "Global MSD",
       subtitle = sprintf("alpha = %.2f (1=diffusive, 2=ballistic)", alpha_global),
...
save_pdf(fig5, "medaka_dynamics_07_msd.pdf", width = 14, height = 10)
...
# 6a: Thickness over time by zone
...
# 6b: Ingression zone vs control thickness
p6b <- ggplot(thickness_ingr, aes(x = time_epoch, y = thickness_p90, color = location)) +
  geom_line(linewidth = 0.8) + geom_point(size = 1.5) +
  geom_vline(xintercept = INGRESSION_ONSET_FRAME * FRAME_INTERVAL_MIN,
             linetype = "dotted", color = "red") +
  scale_color_manual(values = c("Ingression cluster" = "#D73027", "Same-lat control" = "#4575B4")) +
  labs(title = "Thickness: ingression cluster vs same-lat control",
       x = "Time (min)", y = "Thickness P5-P95 (um)", color = NULL) +
  theme_dynamics()

# 6c: Thickness heatmap — early vs late
early_thick <- thickness_data %>%
  filter(time_min_epoch < max(time_min_epoch) / 3) %>%
  group_by(theta_bin, phi_bin) %>%
  summarise(thickness = mean(thickness_p90, na.rm = TRUE), n = sum(n_cells), .groups = "drop") %>%
  filter(n >= 10) %>% mutate(stage = "Early")

late_thick <- thickness_data %>%
  filter(time_min_epoch > 2 * max(time_min_epoch) / 3) %>%
  group_by(theta_bin, phi_bin) %>%
  summarise(thickness = mean(thickness_p90, na.rm = TRUE), n = sum(n_cells), .groups = "drop") %>%
  filter(n >= 10) %>% mutate(stage = "Late")

thick_map <- bind_rows(early_thick, late_thick)

p6c <- ggplot(thick_map, aes(x = phi_bin, y = theta_bin, fill = thickness)) +
  geom_tile(width = THICK_DPHI, height = THICK_DTHETA) +
  scale_fill_viridis_c(option = "magma", name = "Thickness\nP5-P95 (um)") +
  scale_y_reverse() +
  facet_wrap(~stage) +
...
fig6 <- (p6a + p6b) / (p6c + p6d) +
  plot_annotation(
    title = sprintf("%s — Blastoderm Thickness", SPECIES),
    theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14))
  )
save_pdf(fig6, "medaka_dynamics_08_thickness.pdf", width = 14, height = 12)
...
# (tangential cap cuts produce merged blobs with higher 2D cross-sections).
# ============================================================================
...
if (HAS_NUCLEAR) {
  ...
  # Core z-range is defined per timepoint from nuc_time (z_core_lo, z_core_hi).
  ...
  p9a <- ggplot(nuc_slice_nz, aes(timepoint, z_slice, fill = density_per_um2)) +
    ...
    geom_vline(xintercept = CONTRACTION_TP, linetype = "dashed", color = "red", linewidth = 0.4) +
    labs(title = "Nuclear density per z-slice", x = "Timepoint", y = "Z slice",
         subtitle = "Dashed white = core z-range. Dashed red = contraction onset") + theme_dynamics()
  ...
  fig9 <- (p9a + p9b) / (p9c + p9d) / (p9e + p9f) / (p9g + p9h) +
    plot_annotation(
      ...
      subtitle = sprintf("Left: heatmaps (time x z-slice). Right: z-profiles at 8 timepoints. %d bad tps excluded. Red dashed = contraction onset (tp %d).",
                         length(bad_tps), CONTRACTION_TP),
      theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14),
                    plot.subtitle = element_text(hjust = 0, size = 10, color = "grey50"))
    )
  save_pdf(fig9, "medaka_dynamics_09_nuclear_spatial.pdf", width = 14, height = 20)
  ...
  # Data after CONTRACTION_TP is shown with transparency; conclusions from
  # pre-contraction window only.
  # ==========================================================================
  ...
  # Contraction shading helper — adds vertical line + semi-transparent rect
  add_contraction_vline <- function(p) {
    p +
      annotate("rect",
               xmin = CONTRACTION_TP, xmax = Inf,
               ymin = -Inf, ymax = Inf,
               fill = "red", alpha = 0.06) +
      geom_vline(xintercept = CONTRACTION_TP, linetype = "dashed",
                 color = "red", linewidth = 0.4) +
      annotate("text", x = CONTRACTION_TP + 3, y = Inf,
               label = paste0("contraction (tp", CONTRACTION_TP, ")"),
               hjust = 0, vjust = 1.5, size = 2.5, color = "red", fontface = "italic")
  }
  ...
  p10a <- ggplot(nuc_time, aes(timepoint, n_nuclei_3d)) +
    ...
    labs(title = "Nuclei count (3D)", x = "Timepoint", y = "N nuclei",
         subtitle = sprintf("Grey bands = %d bad tps. Red zone = contraction artefact (tp >= %d)",
                            length(bad_tps), CONTRACTION_TP)) +
    theme_dynamics()
  p10a <- add_bad_tp_shading(p10a)
  p10a <- add_contraction_vline(p10a)
  ...
  p10b <- ggplot(z_frac, aes(timepoint, frac, fill = z_bin)) +
    ...
    theme_dynamics()
  p10b <- add_contraction_vline(p10b)
  ...
  p10c <- ggplot(df_diam_strat, aes(timepoint, diam_nwt, color = z_bin)) +
    ...
    theme_dynamics()
  p10c <- add_contraction_vline(p10c)
  ...
  p10d <- ggplot(nuc_time_clean, aes(timepoint)) +
    ...
    theme_dynamics()
  p10d <- add_contraction_vline(p10d)
  ...
  p10e <- ggplot(df_nn_strat, aes(timepoint, nn_nwt, color = z_bin)) +
    ...
    theme_dynamics()
  p10e <- add_contraction_vline(p10e)
  ...
  p10f <- ggplot(df_bbox, aes(timepoint, value, color = axis)) +
    ...
    theme_dynamics()
  p10f <- add_contraction_vline(p10f)

  fig10 <- (p10a + p10b) / (p10c + p10d) / (p10e + p10f) +
    plot_annotation(
      title = sprintf("%s -- Nuclear Temporal Trends", SPECIES),
      subtitle = sprintf("Total nuclei: %s. Depth = Edge vs Core. %d bad tps. Red = contraction artefact (tp >= %d). 3D%s.",
                         format(nuc_global$total_nuclei_detected, big.mark = ","),
                         length(bad_tps), CONTRACTION_TP, core_3d_label),
      theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14),
                    plot.subtitle = element_text(hjust = 0, size = 10, color = "grey50"))
    )
  save_pdf(fig10, "medaka_dynamics_10_nuclear.pdf", width = 14, height = 14)
...
# ============================================================================
# FIGURE 4: SPATIAL CONTEXT — Zone & Margin Map
# ============================================================================

cat("\n=== FIGURE 4: SPATIAL CONTEXT ===\n")

# Annotation helpers for spatial maps
make_ingr_circle <- function() {
  data.frame(angle = seq(0, 2 * pi, length.out = 100)) %>%
    mutate(phi = INGRESSION_CENTER_PHI + INGRESSION_RADIUS * cos(angle),
           theta = INGRESSION_CENTER_THETA + INGRESSION_RADIUS * sin(angle))
}
ingr_circle <- make_ingr_circle()

add_spatial_landmarks <- function(p) {
  if (!is.na(MARGIN_THETA_LANDMARK)) {
    p <- p + geom_hline(yintercept = MARGIN_THETA_LANDMARK,
                        color = "gold3", linetype = "dashed", linewidth = 0.6)
  }
  p <- p + geom_path(data = ingr_circle, aes(x = phi, y = theta),
                     color = "magenta", linetype = "solid", linewidth = 0.5,
                     inherit.aes = FALSE)
  if (!is.na(INGRESSION_THETA_LM)) {
    ing_pt <- data.frame(phi = INGRESSION_PHI_LM, theta = INGRESSION_THETA_LM)
    p <- p + geom_point(data = ing_pt, aes(x = phi, y = theta),
                        color = "magenta", shape = 4, size = 5, stroke = 2,
                        inherit.aes = FALSE)
  }
  p
}
...
HALF_WIN <- 20
val_centers <- c(
  max(round(INGRESSION_ONSET_FRAME * 0.5), HALF_WIN),
  INGRESSION_ONSET_FRAME,
  min(INGRESSION_ONSET_FRAME + 80, max(spots$FRAME) - HALF_WIN),
  min(INGRESSION_ONSET_FRAME + 180, max(spots$FRAME) - HALF_WIN)
)
...
add_annotations <- function(p) {
  p <- p +
    ...
    geom_path(data = ingr_circle, aes(x = phi, y = theta),
              color = "magenta", linetype = "solid", linewidth = 0.5,
              inherit.aes = FALSE)
  if (!is.na(INGRESSION_THETA_LM)) {
    ing_pt <- data.frame(phi = INGRESSION_PHI_LM, theta = INGRESSION_THETA_LM)
    p <- p + geom_point(data = ing_pt, aes(x = phi, y = theta),
                        color = "magenta", shape = 4, size = 3, stroke = 1.5,
                        inherit.aes = FALSE)
  }
  p
}
...
p9a <- ggplot(val_spots, aes(x = PHI_DEG, y = THETA_DEG, color = zone)) +
  ...
  labs(title = "Zone assignment at four time points",
       subtitle = "Gold solid = margin landmark. Magenta = ingression cluster. Dashed/dotted = zone boundaries.") +
  theme_spatial(8) + theme(legend.position = "right")
p9a <- add_annotations(p9a)
...
# 9c: Depth map (ingression marker)
...
p9c <- ggplot(ingr_tiles, aes(x = phi_bin, y = theta_bin, fill = mean_depth)) +
  geom_tile(width = TILE_DPHI, height = TILE_DTHETA) +
  scale_fill_viridis(option = "inferno", name = "Depth\n(um)",
                     limits = c(0, 60), oob = scales::squish) +
  scale_y_reverse() + facet_time + spatial_labs +
  labs(title = "Spherical depth (cumulative ingression marker)",
       subtitle = "Deeper (brighter) regions indicate ingressing cells.") +
  theme_spatial(8) + theme(legend.position = "right")
p9c <- add_annotations(p9c)

# 9d: Depth anomaly
ingr_tiles <- ingr_tiles %>%
  group_by(time_label, theta_bin) %>%
  mutate(lat_mean_depth = mean(mean_depth, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(depth_anomaly = mean_depth - lat_mean_depth)

da_lim <- quantile(abs(ingr_tiles$depth_anomaly), 0.99, na.rm = TRUE) * 1.2

p9d <- ggplot(ingr_tiles, aes(x = phi_bin, y = theta_bin, fill = depth_anomaly)) +
  geom_tile(width = TILE_DPHI, height = TILE_DTHETA) +
  scale_fill_gradient2(low = "#2166AC", mid = "grey90", high = "#D73027",
                       midpoint = 0, limits = c(-da_lim, da_lim), oob = scales::squish,
                       name = "Depth\nanomaly\n(um)") +
  scale_y_reverse() + facet_time + spatial_labs +
  labs(title = "Depth anomaly (tile depth minus latitude mean)",
       subtitle = "Red = deeper than same-latitude average = ingressing cells.") +
  theme_spatial(8) + theme(legend.position = "right")
p9d <- add_annotations(p9d)

fig9 <- (p9a) / (p9b + plot_spacer()) / (p9c) / (p9d) +
  plot_annotation(
    title = sprintf("%s -- Spatial Context & Ingression Maps", SPECIES),
    subtitle = "Gold lines = zone boundaries. Magenta circle = auto-detected ingression cluster.",
    theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14),
                  plot.subtitle = element_text(hjust = 0, size = 9, color = "grey40"))
  )
save_pdf(fig9, "medaka_dynamics_04_spatial_context.pdf", width = 16, height = 24)
...
flow_field <- spots_vel %>%
  ...
  summarise(
    ...
    mean_conv_dorsal = mean(convergence_toward_dorsal, na.rm = TRUE),
    mean_v_depth = mean(v_depth, na.rm = TRUE),
    mean_speed   = mean(inst_speed, na.rm = TRUE),
    n = n(), .groups = "drop"
  ) %>%
...
p10a <- ggplot(flow_field) +
  ...
  labs(title = "Velocity vector field",
       subtitle = "Arrow = tangential velocity. Color = speed.") +
  theme_spatial()
p10a <- add_spatial_landmarks(p10a)
...
p10b <- ggplot(flow_field, aes(x = phi_bin, y = theta_bin, fill = mean_speed)) +
  ...
  theme_spatial()
p10b <- add_spatial_landmarks(p10b)

# 10c: Depth velocity map (ingression signal)
p10c <- ggplot(flow_field, aes(x = phi_bin, y = theta_bin, fill = mean_v_depth)) +
  ...
  scale_fill_gradient2(low = "#2166AC", mid = "grey20", high = "#D73027",
                       ...
                       name = "V_depth\n(um/min)") +
  scale_y_reverse() + spatial_labs +
  labs(title = "Depth change rate (ingression signal)",
       subtitle = "Red = deepening (ingressing). Blue = surfacing.") +
  theme_spatial()
p10c <- add_spatial_landmarks(p10c)

# 10d: Convergence toward dorsal
p10d <- ggplot(flow_field, aes(x = phi_bin, y = theta_bin, fill = mean_conv_dorsal)) +
  geom_tile(width = PHI_BIN, height = THETA_BIN) +
  scale_fill_gradient2(low = "#B2182B", mid = "grey20", high = "#1B7837",
                       midpoint = 0, name = "Conv.\n(um/min)") +
  scale_y_reverse() + spatial_labs +
  labs(title = "Convergence toward dorsal midline",
       subtitle = sprintf("Green = toward dorsal (phi=%.0f). Red = away.", INGRESSION_CENTER_PHI)) +
  theme_spatial()
p10d <- add_spatial_landmarks(p10d)

# 10e: Epiboly velocity map
p10e <- ggplot(flow_field, aes(x = phi_bin, y = theta_bin, fill = mean_v_epi)) +
  geom_tile(width = PHI_BIN, height = THETA_BIN) +
  ...
  theme_spatial()
p10e <- add_spatial_landmarks(p10e)

# 10f: Movement type spatial map
type_spatial <- spots_vel %>%
  ...
  summarise(
    ...
    n_con = sum(movement_type == "Convergence"),
    n_ing = sum(movement_type == "Ingression"),
    n_total = n(), .groups = "drop"
  ) %>%
  filter(n_total >= FLOW_MIN_N) %>%
  mutate(dominant = case_when(
    n_ing >= n_epi & n_ing >= n_ani & n_ing >= n_con ~ "Ingression",
    n_con >= n_epi & n_con >= n_ani                  ~ "Convergence",
    n_epi >= n_ani                                    ~ "Epiboly",
    TRUE                                              ~ "Animalward"
  ))
...
p10f <- ggplot(type_spatial, aes(x = phi_bin, y = theta_bin, fill = dominant)) +
  geom_tile(width = PHI_BIN, height = THETA_BIN) +
  ...
  scale_y_reverse() + spatial_labs +
  labs(title = "Dominant movement type per tile (majority rule)",
       subtitle = "Arrows = mean velocity. Ingression = depth change + spatial proximity to cluster.") +
  theme_spatial()
p10f <- add_spatial_landmarks(p10f)

# 10g: Flow field at 3 time points
time_points <- c(round(max(spots$FRAME) * 0.1),
                 round(max(spots$FRAME) * 0.5),
                 round(max(spots$FRAME) * 0.9))
time_labels_ordered <- sprintf("t = %.0f min", frame_to_min(time_points))

flow_timepoints <- lapply(seq_along(time_points), function(i) {
  t <- time_points[i]
  spots_vel %>%
...
# 10h: Movement type evolution over time
type_time <- spots_vel %>%
  left_join(track_metrics %>% dplyr::select(TRACK_ID, movement_type), by = "TRACK_ID") %>%
  filter(!is.na(movement_type)) %>%
  mutate(time_bin = floor(time_min / 5) * 5) %>%
...
p10h <- ggplot(type_time, aes(x = time_bin, y = frac, fill = movement_type)) +
  geom_area() +
  scale_fill_manual(values = mvt_colors, name = "Type") +
  labs(title = "Movement type fraction over time", x = "Time (min)", y = "Fraction") +
  theme_dynamics()

fig10 <- (p10a + p10b) / (p10c + p10d) / (p10e + p10f) / (p10g + p10h) +
  plot_annotation(
    title = sprintf("%s -- Spatial Flow Summary", SPECIES),
    subtitle = "Time-averaged velocity fields on embryo surface. Gold dashed = margin. Magenta = ingression.",
    theme = theme(plot.title = element_text(hjust = 0, face = "bold", size = 14),
                  plot.subtitle = element_text(hjust = 0, size = 8, color = "grey40"))
  )
save_pdf(fig10, "medaka_dynamics_05_spatial_flow.pdf", width = 14, height = 20)
...
write_csv(track_metrics, file.path(OUTPUT_DIR, "medaka_dynamics_track_metrics.csv"))
...
write_csv(summary_data, file.path(OUTPUT_DIR, "medaka_dynamics_summary.csv"))

# Save thickness data for comparison
write_csv(thickness_zone_time %>% mutate(species = SPECIES),
          file.path(OUTPUT_DIR, "medaka_thickness_data.csv"))

cat("\n")
cat(strrep("=", 70), "\n")
cat("  MEDAKA ANALYSIS COMPLETE\n")
cat(strrep("=", 70), "\n")
