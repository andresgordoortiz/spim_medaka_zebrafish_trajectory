...
vdens_dt[, epoch := factor(epoch, levels = c("Frame 0", sprintf("Frame %d", ENDPOINT_FRAME)))]
...
p3c_vol <- ggplot(fc_vol, aes(x = y_bin, y = log2_fc, colour = region)) +
  ...
  geom_point(size = 2.5) +
  scale_colour_manual(values = c("ROI" = "#D95F02", "Outside" = "grey30"), name = "") +
  labs(title = sprintf("Vol. Density Fold Change (frame 0 vs %d)", ENDPOINT_FRAME),
...
p3c_nn <- ggplot(fc_nn, aes(x = y_bin, y = log2_fc, colour = region)) +
  ...
  geom_point(size = 2.5) +
  scale_colour_manual(values = c("ROI" = "#D95F02", "Outside" = "grey30"), name = "") +
  labs(title = sprintf("Geodesic Density Fold Change (frame 0 vs %d)", ENDPOINT_FRAME),
...
save_pdf(p3, "zf_density_03_density_fold_change.pdf", width = 22, height = 16)
